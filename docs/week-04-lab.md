...
## 6. 수업 중 개발 사례 2건으로 Prompt 최적화 과정 보기

전체 30건은 수업 중 다시 실행하지 않는다. 외부 전송, 모델·가격과 남은 요청 수를 확인한
경우에만 튜터가 개발 문제 첫 2건을 실제 API로 실행한다. DeepEval GEPA가 요구하는 최소
사례 수다. 전체 명령과 중단 조건은 Week 4 튜터 runbook에 있다.
...
`selected=null`은 실패가 아니라 의도한 결과다. 최종 선택과 품질 분석은 3절에서 읽은 개발
18건·검증 6건 전체 저장 기록만 사용한다. 이미지 API도 수업 중 다시 호출하지 않고, 5절의
저장 응답 5건을 사용한다. 승인이 없거나 API 서비스가 중단되면 시연을 생략하고 저장 기록으로
계속한다.
...
전체 기록의 검증 평균은 여전히 처음 지시문 `0.2950`, 후보 `0.2459`이므로 수업에서는 처음
지시문을 유지한다.
