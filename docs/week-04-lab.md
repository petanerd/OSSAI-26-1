...
## 6. 수업 중 개발 사례 1~2건으로 Prompt 최적화 과정 보기

전체 30건은 수업 중 다시 실행하지 않는다. 외부 전송, 모델·가격과 남은 요청 수를 확인한
경우에만 튜터가 개발 문제 첫 1건을 실제 API로 실행한다. 시간이 충분할 때만 2건으로 늘린다.
전체 명령과 중단 조건은 Week 4 튜터 runbook에 있다.
...
- 수업 중 개발 사례 1~2건은 지시문 생성 과정만 확인하고 후보 선택에 쓰지 않았다.
